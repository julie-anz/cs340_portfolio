...
-- insert a resource
DROP PROCEDURE IF EXISTS sp_insert_resource;
DELIMITER //
...
BEGIN
  ...
  VALUES(
     (SELECT userID FROM Users WHERE userID = p_id),
     p_name,
     p_description
  );
...
DROP PROCEDURE IF EXISTS sp_insert_resourceLocation;
DELIMITER //
CREATE PROCEDURE sp_insert_resourceLocation(
  IN resourceId int,
  IN locationId int
  )
BEGIN
...
CREATE PROCEDURE sp_update_resource(
  IN id int,
  IN updateData VARCHAR(300)
  )
BEGIN
 
  Update Resources
     SET resourceDescription = updateData
     WHERE resourceID = id;
  -- end the stored procedure
...
DROP PROCEDURE IF EXISTS sp_update_resourceLocation;
DELIMITER //
CREATE PROCEDURE sp_update_resourceLocation(
  IN id int,
  IN updateData int
  )
BEGIN
  ...
  UPDATE ResourceLocations
     SET
     locationID = (SELECT locationID FROM Locations WHERE locationID = updateData)
     WHERE
     ResourceLocationsID =  id;
  -- end the stored procedure
...
DELIMITER //
CREATE PROCEDURE sp_delete_loan(IN id int)
BEGIN
  -- Delete a Loan
  DELETE FROM Loans
     WHERE loanID = id;
...
DELIMITER //
CREATE PROCEDURE sp_delete_location(IN id int)
BEGIN
  -- Delete a Location
...
-- ***** this is the DLL file wrapped in a reset procedure *****
